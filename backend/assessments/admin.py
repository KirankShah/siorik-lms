...
class CategoryBucketInline(admin.TabularInline):
    model = CategoryBucket
    extra = 0


class CategorizeItemInline(admin.TabularInline):
    model = CategorizeItem
    extra = 0


class HotspotRegionInline(admin.TabularInline):
    model = HotspotRegion
    extra = 0
...
class QuizAdmin(admin.ModelAdmin):
    ...
    search_fields = ('title', 'slide__title')
