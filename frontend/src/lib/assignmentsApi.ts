...
// A Slide of type ASSIGNMENT has at most one Assignment (slide is OneToOne on
// the backend) — null means none has been created yet.
export async function fetchAssignmentForSlide(slideId: number): Promise<Assignment | null> {
  const assignments = await apiFetch<Assignment[]>(`/assignments/?slide=${slideId}`)
  return assignments[0] ?? null
}

// Unfiltered list — scoped server-side to assignments in courses the caller
// can see (editable courses for admins, visible courses for everyone else).
