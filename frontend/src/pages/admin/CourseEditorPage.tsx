...
export function CourseEditorPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      if (isCreateMode) {
        ...
        navigate(`/admin/courses/${created.slug}/settings`, { replace: true })
      } else {
  ...
  if (!isCreateMode && !course) return <p className="text-sm text-neutral-500">Loading course…</p>

  return (
    <div className="space-y-6">
      <Card>
        <h1 className="text-lg font-semibold text-neutral-900">{isCreateMode ? 'New Course' : 'Course Details'}</h1>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div>
            <p className="text-sm font-medium text-neutral-700">Content owner</p>
          ...
          {isPlatformAdmin && (
            <div>
              <label className="block text-sm font-medium text-neutral-700">Organization (metadata only)</label>
              <select
          ...
          <Input
            id="course-deadline"
            label="Completion deadline (days after enrollment)"
            type="number"
            min={1}
        ...
        <>
          <Card>
            <div className="flex items-center justify-between">
              ...
              <Link to={`/admin/courses/${course.slug}/slides`} className="text-sm font-medium text-brand-navy hover:underline">
                Open Slides →
              </Link>
            </div>
            <p className="mt-1 text-xs text-neutral-400">
              Manage modules and lessons here. Add, edit, and reorder each lesson's slides in the Slides tab.
            </p>
            <div className="mt-4">
              <CurriculumBoard course={course} onChanged={() => loadCourse(course.slug)} />
            </div>
            ...
            {isAddingModule ? (
              <div className="mt-4 flex items-center gap-2">
                ...
                />
                <button type="button" onClick={handleAddModule} className="text-sm font-medium text-brand-navy">
                  Add
                </button>
                <button type="button" onClick={() => setIsAddingModule(false)} className="text-sm text-neutral-500">
                  Cancel
                </button>
              </div>
            ) : (
